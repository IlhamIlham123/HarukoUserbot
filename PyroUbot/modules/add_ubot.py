...
async def _(client, message):
    ...
    pantek = "https://files.catbox.moe/5xbu55.jpg"
...
ɪᴅ : {new_client.me.id}
ᴘʀᴇꜰɪxᴇꜱ : {' '.join(SH)}
ʜᴀʀᴀᴘ ᴊᴏɪɴ : @titlenyaaja ʜᴀʀᴀᴘ ᴊᴏɪɴ ʀᴏᴏᴍ ᴅɪᴀᴛᴀs ᴅᴀɴ ᴊᴀɴɢᴀɴ ᴏᴜᴛ ᴀɢᴀʀ sᴀғᴇᴛʏ
ᴊɪᴋᴀ ʙᴏᴛ ᴛɪᴅᴀᴋ ʀᴇꜱᴘᴏɴ, ᴋᴇᴛɪᴋ /restart</b></blockquote>
    ...
    try:
        await new_client.join_chat("titlenyaaja")
        await new_client.join_chat("titlenyaaja")
        await new_client.join_chat("titlenyaaja")
...
async def _(client, callback_query):
    ...
    if user_id not in admin_users:
        return await callback_query.answer(
            f"❌ Tombol ini bukan untuk kamu {callback_query.from_user.first_name} {callback_query.from_user.last_name or ''}",
            show_alert=True,
        )
    ...
    if len(data_parts) < 2:
        return await callback_query.answer(
            "❌ Data tidak valid.", show_alert=True
        )
    target_id_str = data_parts[1]
    try:

        show = await bot.get_users(target_id_str)
        target_id = show.id
    ...
    for X in ubot._ubot:
        if target_id == X.me.id:
            ...
            await callback_query.answer(
                f"✅ {target_mention} berhasil dihapus dari database.", show_alert=True
            )

            if ubot._ubot:
                first_bot_id = ubot._ubot[0].me.id
                markup = InlineKeyboardMarkup(BTN.UBOT(first_bot_id, 0))
                text = await MSG.UBOT(0)
            else:
                markup = None
                text = "✅ Semua userbot telah dihapus."
    ...
    await callback_query.answer(
        "❌ Userbot tidak ditemukan atau sudah logout.",
        show_alert=True,
    )
